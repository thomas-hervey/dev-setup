...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  echo "Good, we will begin by checking the current local hostname configurations by opening etc/hosts."
fi;
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    read -p "*Check and see if the key 'id_rsa' exists. If so, the following command will overwrite it. Continue? (y)"
    if [[ $REPLY =~ ^[Yy]$ ]]; then
